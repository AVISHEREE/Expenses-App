...
try {
   const [results] = await pool.query(`INSERT INTO 
                                       ...
                                       VALUES(?,?,?,?,?)`,[user_id,amount,date,type,description]);
    return results;
} catch (err) {
    return err
}
...
const getAllExpenseForAnUser = async (userId) =>{
    try {
        const [results] = await pool.query(`SELECT e.id , e.amount , e.date , e.type , e.description 
                                      FROM expenses e
                                      JOIN users u
...
const deleteSingleExpense = async (expenseId) =>{
    try {
        const [results] = await pool.query(`DELETE FROM expenses WHERE id = ?`,[expenseId]);
        return results;
    } catch (err) {
        return err
    }
...
const updateExpense = async (whatFeildToUpdate,valueToUpdate,expenseId) =>{
  try {
    const results = await pool.query(`UPDATE expenses
                                      SET ${whatFeildToUpdate} = ? 
                                      WHERE id = ?`,[valueToUpdate,expenseId]);
    return results;
  } catch (err) {
    return err
  }  
